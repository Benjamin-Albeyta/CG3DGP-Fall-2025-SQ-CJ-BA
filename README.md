...
- ### Added a death UI that you are taken too after dying with options and unique music
Part of larger changes to the death system, a delay after death, a specific death sound effect, and no longer and instant restart, instead the death sytems are handled in a new script unrelated to GameManager.cs, handled in PlayerDeathHandler.cs. Has a delay after dying where control is locked out from the player, then stops music, plays death sfx and brings to a death menu with it's own options, to restart the level or return to the menu.
![Death UI](https://cdn.bsky.app/img/feed_fullsize/plain/did:plc:gw3cmasus5q2obg274yrww7u/bafkreibez7ig2wxtjgcaglmndw3zae4pli77bjukdeo5m4e62wsgirrb3y@jpeg)
...
![Level 3 Overview](https://cdn.bsky.app/img/feed_fullsize/plain/did:plc:gw3cmasus5q2obg274yrww7u/bafkreiargyd3x7tqdooplrewwrhr43b6hev6dwz6os3azle7ylexrrqb5m@jpeg)
...
### Sophia Qian:
Implemented transparent blocks, the GameManger.cs system and its inital implementation, worked on level design for Level 2. Created and assisted with construction of Mac and Web builds. Worked on optimization and how well the game runs. Wrote scripts for the door and level completion, as well as trigger-based moving platforms that activate when the player steps on them. Implemented enemy chase behavior on top of the patrol system, added per-level level text showing the current level, created an intro scene between the main menu and Level 1, and helped design the Level 3 potion that slows the alternating platforms to better balance the overall difficulty.
